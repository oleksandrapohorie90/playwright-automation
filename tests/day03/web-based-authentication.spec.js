...
test("Bypass authentication by embedding the credetials in the url", async ({
  ...
  //https://username:password@the-internet-5chk.onrender.com/basic_auth <- thi sis the format to embed the credentials in the url,
  // but it is not recommended for security reason since we expose username and password
}) => {
...
test("Bypass authentication by Encoding the credentials base64 format", async ({
  ...
  //we use a map to set the header of the request, we can use the setExtraHTTPHeaders method to set the header of the request,
  //and then we can pass the encoded
